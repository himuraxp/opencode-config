...
# setup.sh — First-time installation of opencode-config on a new machine.
#
# This script:
#   1. Checks prerequisites (Node.js, npm)
#   2. Installs external dependencies (opencode-ai, rtk)
#   3. Runs install.sh (agents, standards, frameworks, config files)
#   4. Installs npm dependencies for plugins
#   5. Collects environment variables interactively (secrets)
...
if command -v opencode &>/dev/null; then
  ok "opencode-ai already installed: $(opencode --version 2>/dev/null || echo 'unknown')"
else
